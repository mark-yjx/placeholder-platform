...
test('account status bar shows a visible sign-in entry point before login', () => {
  ...
  assert.equal(item.text, '$(account) Placeholder');
  assert.equal(item.tooltip, 'Open Placeholder account and sign in to start practicing');
...
test('account status bar shows the student email after login', () => {
  ...
  assert.equal(item.text, '$(account) Placeholder');
  assert.equal(item.tooltip, 'Signed in as student1@example.com. Open Placeholder account and stats');
  assert.equal(item.shown, 1);
});
